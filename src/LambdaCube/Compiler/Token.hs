...
makeTokenParser :: (Stream s m (Char, Indentation))
                => (forall a . ParsecT (IndentStream s) st m a -> ParsecT (IndentStream s) st m a)
                -> GenLanguageDef (IndentStream s) st m -> GenTokenParser (IndentStream s) st m
makeTokenParser lexeme languageDef
