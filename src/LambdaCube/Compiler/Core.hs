...
getFunDef' a t = fn
  where
    fn = FunName a (DeltaDef d) t
    d = getFunDef a $ \xs -> Neut $ Fun fn (reverse xs) Delta
...
mkFunc (Neut (Fun (FunName n (ExpDef def) ty) xs (Neut RHS_{}))) | Just def' <- removeLams (length xs) def = Just (n, def', ty, xs)
mkFunc _ = Nothing
